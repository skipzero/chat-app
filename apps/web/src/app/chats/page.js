...
import { getRoomMessages, getRooms, createRoom } from "../../lib/api";
...
export default function ChatPage() {
    ...
    return (<div className="grid lg:grid-cols-[280px_1fr] gap-4 p-4 max-w-6xl mx-auto h-[calc(100svh-4rem)]">
      <aside className="flex flex-col gap-4 rounded-lg border border-border p-4 bg-panel">
        ...
        <div className="space-y-2 overflow-y-auto">
          ...
          {rooms.map((room) => (<button key={room._id} type="button" onClick={() => handleRoomSelect(room._id)} className={`block w-full text-left rounded-md border px-3 py-2 ${room._id === activeRoomId ? "border-primary bg-primary/10" : "border-border bg-background"}`}>
        ...
        <form onSubmit={handleCreateRoom} className="space-y-3 pt-4 border-t border-muted/20">
          <div>
            ...
            <Input id="room-name" value={newRoomName} onChange={(event) => setNewRoomName(event.target.value)} placeholder="Room name" className="w-full"/>
          </div>
          <div>
            <Input id="room-description" value={newRoomDescription} onChange={(event) => setNewRoomDescription(event.target.value)} placeholder="Description (optional)" className="w-full"/>
          </div>
      ...
      <section className="flex flex-col rounded-lg border border-border bg-panel p-4">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 rounded-lg border border-background p-3">
          ...
          {messages.length === 0 ? (<div className="text-center text-muted-foreground py-20">No messages yet in this room.</div>) : (messages.map((message) => (<div key={message._id} className={`rounded-2xl px-4 py-3 ${message.senderId === userId ? "bg-primary/10 ml-auto text-right" : "bg-secondary/10 mr-auto text-left"}`}>
        ...
        <form onSubmit={handleSubmit} className="mt-4 flex items-center gap-2">
          <Input value={input} onChange={(event) => setInput(event.target.value)} placeholder={activeRoom ? "Type a message..." : "Select a room first"} className="flex-1" disabled={!activeRoom} autoComplete="off"/>
